---
id: day-28-mock-review-and-final-readiness-check
title: Day 28 — Mock Review and Final Readiness Check
domain: integrated-capstone-assessment
level: capstone
jurisdiction: AU-NZ
...
![A learner sorting one mock into useful review categories instead of placing one score on top](../../../assets/learning/4-week/day-28-mock-review-and-final-readiness-check/sort-the-error-not-just-the-score.svg)
...
```mermaid
flowchart TD
    A["Preserve original answer confidence and time"] --> B["Evaluate result process safety source communication and time"]
    B --> C{"Exact claim or criterion involved?"}
    C -- Yes --> D["Verify with current authorised source"]
    C -- No --> E["Classify error mechanism and consequence"]
    D --> E
    E --> F["Apply smallest useful correction"]
    F --> G["Re-attempt one varied task"]
    G --> H{"Correction holds without prompting?"}
    H -- Yes --> I["Record readiness state and evidence"]
    H -- No --> J["Add focused remediation and mark not yet ready"]
```
...
```mermaid
flowchart LR
    A["High confidence plus critical consequence"] --> P1["Priority 1 immediate remediation and qualified review"]
    B["Low confidence plus critical consequence"] --> P2["Priority 2 source-guided remediation"]
    C["High confidence plus material consequence"] --> P2
    D["Low confidence plus material consequence"] --> P3["Priority 3 targeted practice"]
    E["Minor consequence"] --> P4["Priority 4 efficiency or presentation improvement"]
```
...
![A learner choosing three precise repair cards rather than carrying every note into the final review](../../../assets/learning/4-week/day-28-mock-review-and-final-readiness-check/three-repairs-not-thirty-notes.svg)
