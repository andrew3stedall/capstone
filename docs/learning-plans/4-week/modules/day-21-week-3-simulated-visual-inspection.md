...
![Stick figure using a planned route through an installation instead of chasing a single obvious defect](../../../assets/learning/4-week/day-21-week-3-simulated-visual-inspection/follow-the-inspection-route.svg)
...
```mermaid
flowchart TD
    A["Define scope evidence and prohibited actions"] --> B["Map sources functions and operating state"]
    B --> C["Scan each functional inspection zone"]
    C --> D["Record objective observations"]
    D --> E{"Immediate danger indicator or boundary exceeded?"}
    E -- Yes --> F["Stop and escalate"]
    E -- No --> G["Identify evidence gaps and applicable source"]
    G --> H["Assign provisional response priority"]
    H --> I["Transfer bounded inspection record"]
```
...
```mermaid
flowchart LR
    O["Observation: what is visible"] --> G["Gap: what is not established"]
    G --> Q["Question: what requirement or evidence is needed"]
    Q --> P["Provisional concern and response priority"]
    P --> V["Qualified verification outside this simulation"]
    O -.->|"do not jump"| V
```
...
![Stick figure writing “observed” on one card and placing “cause unknown” on a separate card](../../../assets/learning/4-week/day-21-week-3-simulated-visual-inspection/separate-evidence-from-conclusion.svg)
