---
id: day-01-exam-orientation-and-wiring-rules-navigation
title: Day 1 — Exam Orientation and Wiring Rules Navigation
...
quality_passes:
  - date: 2026-07-14
    scope: source-hierarchy, evidence-quality and remediation pass
prerequisites:
...
![A stick figure choosing the controlling source rather than searching every book at once](../../../assets/learning/4-week/day-01-exam-orientation-and-wiring-rules-navigation/choose-the-source.svg)
...
```mermaid
flowchart TD
    A["Read the scenario"] --> B{"What controls this question?"}
    B -->|"Assessment conditions"| C["Current RTO instructions or assessor direction"]
    B -->|"Technical requirement"| D["Authorised law, standard, regulator or network source"]
    B -->|"Equipment-specific matter"| E["Manufacturer information plus governing sources"]
    B -->|"Work method"| F["Approved workplace procedure plus governing duties"]
    C --> G["Extract object, action and condition"]
    D --> G
    E --> G
    F --> G
    G --> H["Use contents, index, headings and defined terms"]
    H --> I["Read scope, exceptions and cross-references"]
    I --> J{"Applicable and complete?"}
    J -->|"No"| K["Change search level or source family"]
    K --> G
    J -->|"Yes"| L["Explain in original words and record evidence trail"]
```
...
```mermaid
flowchart BT
    A["Memory, snippet or informal note"] --> B["Search aid only"]
    B --> C["Relevant secondary explanation"]
    C --> D["Authorised current source located"]
    D --> E["Scope, definitions and references checked"]
    E --> F["Scenario-specific conclusion with evidence trail"]
```
...
![A stick figure following cross-references instead of stopping at the first search hit](../../../assets/learning/4-week/day-01-exam-orientation-and-wiring-rules-navigation/follow-the-reference.svg)
...
flowchart LR
    ...
    F --> G["Different controlling source family"]
```
...
![A learner climbing from a vague search phrase to a verified source trail](../../../assets/learning/4-week/day-01-exam-orientation-and-wiring-rules-navigation/climb-the-evidence-ladder.svg)
