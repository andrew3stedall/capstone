...
![A learner moves through timed scenario stages while keeping a protected review reserve](../../../assets/learning/12-week/timed-integrated-scenario-with-worked-example-fading-removed/protect-the-review-reserve.svg)
...
```mermaid
flowchart LR
  A[Time plan and review reserve] --> B[Scenario decomposition]
  B --> C[Dependency-ordered response]
  C --> D[Evidence and decision ledger]
  D --> E{Time gate reached?}
  E -->|Yes| F[Move to next required deliverable]
  E -->|No| C
  F --> G[Final integrated review]
  G --> H[Bounded submission and remediation notes]
```
