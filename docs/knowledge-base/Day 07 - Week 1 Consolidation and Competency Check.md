...
```mermaid
flowchart TD
    A["Facts and gaps"] --> B["Possible supplies"]
    B --> C["Technical classification"]
    C --> D["Governing source family"]
    D --> E["Functions and current paths"]
    E --> F["Claim grade"]
    F --> G["Bounded conclusion"]
    G --> H["Readiness or remediation"]
```
